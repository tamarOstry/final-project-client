...
export default function SearchBaby() {
  ...
  useEffect(() => {
    getIdentities().then(data => {
      console.log(data, "data in SearchBaby");
      setIdentitiesNumbers(data);
    })
  }, []);

  function goToHistory(opt) {
    console.log(opt,"opt");
  ...
  return (
    <div className='searchInput'>
      ...
      {/* <h2>hi {sessionStorage.getItem('nurse').firstName}</h2> */}
      <Autocomplete
        disablePortal
        id="combo-box-demo"
      ...
      />
      <button onClick={(opt)=>goToHistory(opt)}>insert</button>
    </div>
  );
